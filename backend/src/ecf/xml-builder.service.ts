...
export class XmlBuilderService {
  buildEcfXml(factura: any, detalles: any[], emisor: any, comprador: any): string {
    ...
    idDoc.ele('FechaVencimientoSecuencia').txt(
      this.formatDate(new Date(new Date().setFullYear(new Date().getFullYear() + 1))),
    );
    ...
    totales.ele('MontoGravadoTotal').txt(this.formatMoney(factura.monto_gravado_18 + factura.monto_gravado_16));
    totales.ele('MontoGravadoI1').txt(this.formatMoney(factura.monto_gravado_18));
    if (factura.monto_gravado_16 > 0) {
      totales.ele('MontoGravadoI2').txt(this.formatMoney(factura.monto_gravado_16));
    }
    if (factura.monto_exento > 0) {
      totales.ele('MontoExento').txt(this.formatMoney(factura.monto_exento));
    }
    totales.ele('ITBIS1').txt('18');
    totales.ele('TotalITBIS1').txt(this.formatMoney(factura.itbis_18));
    if (factura.itbis_16 > 0) {
      totales.ele('ITBIS2').txt('16');
      totales.ele('TotalITBIS2').txt(this.formatMoney(factura.itbis_16));
    }
    totales.ele('TotalITBIS').txt(this.formatMoney(factura.itbis_total));
    totales.ele('MontoTotal').txt(this.formatMoney(factura.monto_total));
    ...
    for (const item of detalles) {
      ...
      itemNode.ele('PrecioUnitarioItem').txt(this.formatMoney(item.precio_unitario));

      if (item.descuento_monto > 0) {
        itemNode.ele('DescuentoMonto').txt(this.formatMoney(item.descuento_monto));
      }

      if (item.tasa_itbis > 0) {
        itemNode.ele('TablaSubDescuento');
        itemNode.ele('TasaITBIS').txt(String(item.tasa_itbis));
        itemNode.ele('ITBISItem').txt(this.formatMoney(item.itbis_monto));
      }

      itemNode.ele('MontoItem').txt(this.formatMoney(item.monto_total));
    }
