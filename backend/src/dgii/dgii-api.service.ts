...
@Injectable()
export class DgiiApiService {
  private env = ENVIRONMENTS.TesteCF;
  ...
  async authenticate(signedSeedXml: string): Promise<string> {
    try {
  ...
  async sendEcf(signedEcfXml: string, rncEmisor: string, encf: string) {
    if (!this.token) {
  ...
  async simulateSend(encf: string) {
  ...
  async simulateCheck(trackId: string) {
